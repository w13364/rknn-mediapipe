 ...
 */
 ...
 Args parse_args(int argc, char* argv[]) {
     ...
     for (int i = 1; i < argc; ++i) {
         ...
         if (a == "-i" || a == "--input") { args.input = argv[++i]; }
         ...
         else if (a == "-o" || a == "--output-video") { args.output_video = argv[++i]; }
         else if (a == "-h" || a == "--help") {
             std::cout << "                                                                                                          " << std::endl;
             std::cout << "usage: blaze_detect_live [-h] [-i INPUT] [-t] [-b BLAZE] [-m MODEL1] [-n MODEL2] [-d] [-w] [-z] [-y] [-f] [-o OUTPUT_VIDEO] " << std::endl;
             std::cout << "                                                                                                          " << std::endl;
             ...
             std::cout << "  -o,        --output-video   Path of output video file. Default is not saving video                       " << std::endl;
             std::cout << "                                                                                                          " << std::endl;
             exit(0);
         }
     }
 ...
 int main(int argc, char* argv[]) {
     ...
     std::cout << " --output-video: " << args.output_video << std::endl;
     ...
     if (!args.output_video.empty()) {
         ...
         // 打开视频写入器
         video_writer.open(args.output_video, fourcc, fps, cv::Size(frame_width, frame_height));
         
         if (!video_writer.isOpened()) {
             std::cerr << "[ERROR] Could not open video writer: " << args.output_video << std::endl;
         } else {
             std::cout << "[INFO] Saving video to: " << args.output_video << " (" << fps << "fps)" << std::endl;
         }
     }
